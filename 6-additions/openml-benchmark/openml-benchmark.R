...
source("6-additions/openml-benchmark/openml-benchmark-functions.R")
n.reps <- 10
...
# saveRDS(task.ids, "6-additions/openml-benchmark-task-ids.RDS")
# if (multi.2cls.switch == "multi") {
#   task.ids <- OpenML::listOMLTasks(
#     number.of.missing.values = 0, tag = "OpenML100",
#     estimation.procedure = "10-fold Crossvalidation")$task.id %>% 
#     setdiff(task.ids)
#   saveRDS(task.ids, "6-additions/openml-benchmark-categ-task-ids.RDS")
# }
task.ids <- switch(
  multi.2cls.switch,
  "multi" = readRDS("6-additions/openml-benchmark/openml-benchmark-categ-task-ids.RDS"),
  "2cls"  = readRDS("6-additions/openml-benchmark/openml-benchmark-task-ids.RDS"))
tasks <- switch(
  multi.2cls.switch, 
  "multi" = readRDS("6-additions/openml-benchmark/openml-benchmark-multi-cls-tasks.RDS"), 
  "2cls"  = readRDS("6-additions/openml-benchmark/openml-benchmark-2-cls-tasks.RDS"))
...
#          "multi" = "6-additions/openml-benchmark-time-ests-categ.RDS", 
#          "2cls" = "6-additions/openml-benchmark-time-ests.RDS") %>% 
#     saveRDS(time.estimate, .)
# }
time.estimate <- switch(
  multi.2cls.switch, 
  "multi" = readRDS("6-additions/openml-benchmark/openml-benchmark-time-ests-categ.RDS"), 
  "2cls"  = readRDS("6-additions/openml-benchmark/openml-benchmark-time-ests.RDS"))
...
for (run in 1:n.reps) {
  ...
  switch(
    multi.2cls.switch, 
    "multi" = "6-additions/openml-benchmark/openml-benchmark-multi-cls-repeated-run.RDS", 
    "2cls"  = "6-additions/openml-benchmark/openml-benchmark-2-cls-repeated-run.RDS") %>% 
