...
DisplayResults <- function(multi.2cls.switch, repeated) {
  ...
  ifelse(repeated, "repeated", "single") %>%
    paste0("6-additions/openml-benchmark/oml-acc-", multi.2cls.switch, "-", ., ".eps") %>%
    cowplot::save_plot(all.plots)
  
  aggregate(abs.diff ~ method, mean.results.df, summary, digits = 3) %>% print
  aggregate(rel.gain ~ method, mean.results.df, summary, digits = 3) %>% print
  aggregate(mabt.less.wrong.preds ~ method, mean.results.df, summary, digits = 3) %>% print
}

sink("6-additions/openml-benchmark/openml-2cls-pairwise.txt")
DisplayResults(multi.2cls.switch = "2cls", repeated = TRUE) # Fig 19 (main)
sink()
