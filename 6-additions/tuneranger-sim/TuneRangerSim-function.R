TuneRangerSim <- function(seed, n_obs) {
  ...
  rf_tune <- tuneRanger::tuneRanger(
    classif_task, measure = list(mlr::acc), build.final.model = FALSE)
  ...
  test_boot <- boot::boot(
    test_similar, .ClassBootStat, R = 10000)
  ...
  # bt_ci_est <- rf_preds[, select_res$default$id] %>% 
  #   TiltCi(test_boot, 0.05, "class", ., 0.5, dat$test$Class)
  # bt_ci <- data.frame(method = "bt", lwr = bt_ci_est)
  
  sidak_level <- (1-0.05)^(1/length(preselect_ids))
  
  # bt_sidak_ci_est <- rf_preds[, select_res$proposed$id] %>% 
  #   TiltCi(test_boot, 1-sidak_level, "class", ., 0.5, dat$test$Class)
  # bt_sidak_ci <- data.frame(method = "bt", lwr = bt_sidak_ci_est)
  ...
  # select_res$default$ci <- rbind(default_ci, bt_ci)
  ...
  # sidak_ci <- rbind(sidak_ci, bt_sidak_ci)
