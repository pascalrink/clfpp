...
#   t0 <- Sys.time()
#   eval_boot <- boot::boot(
#     within1SE_similar_mat, .ClassBootStat, R = 10000)
#   alpha <- 0.05
#   measure <- "class"
...
#   return(CtgExampleAccuracy(seed, n.cv = 10))
# }
# t1 <- Sys.time()
# t1-t0
# parallel::stopCluster(par_clust)
# saveRDS(result_list, "6-additions/ctg-example/ctg-example-rcv.RDS")
#######################
# 2.5 mins on 50 cores
#######################
...
sink("6-additions/ctg-example/ctg-results.txt")
comp_results$table[13:18, ]
comp_results_df <- do.call(rbind, comp_results$ci)

aggregate(ci ~ method, comp_results_df, summary)

subset(comp_results_df, maxdiff > 0.02 & method == "wald")

# CtgExampleAccuracy(38,  1) %>% saveRDS("6-additions/ctg-example/ctg-seed-38-1-cv.RDS")
readRDS("6-additions/ctg-example/ctg-seed-38-1-cv.RDS")

# CtgExampleAccuracy(38, 10) %>% saveRDS("6-additions/ctg-example/ctg-seed-38-10-cvs.RDS")
readRDS("6-additions/ctg-example/ctg-seed-38-10-cvs.RDS")

# CtgExampleAccuracy(53, 10) %>% saveRDS("6-additions/ctg-example/ctg-seed-53-10-cvs.RDS")
readRDS("6-additions/ctg-example/ctg-seed-53-10-cvs.RDS")
sink()
