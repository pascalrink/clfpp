...
n_gt <- 20000
damped_true_coeffs <- 0.5 * true_coeffs
gt_feats <- rnorm(n_gt * n_feats) %>% matrix(ncol = n_feats)
gt_labs <- (boot::inv.logit(
  gt_feats %*% damped_true_coeffs) > runif(n_gt)) %>% as.numeric
...
sim_out <- foreach::foreach(
  run = 1:n_runs, .packages = c("dplyr", "glmnet")) %dopar% {
    ...
    eval_labs <- (boot::inv.logit(
      eval_feats %*% damped_true_coeffs) > runif(n_eval)) %>% as.numeric
...
saveRDS(sim_out, "6-additions/conditional-coverage/conditional-amplified-mabt.RDS")
time_1 <- Sys.time()
