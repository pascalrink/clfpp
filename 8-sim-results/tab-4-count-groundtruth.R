...
df_list <- lapply(cis_files, function(.fn) {
  ...
  .df <- subset(.df, cv == TRUE & (method == "mabt" | method == "wilson" & rule == "best") & n_eval == 100)  # specify method ("wilson" or "delong") and n_eval (50, 100, 150) here
  .df <- subset(.df, measure == "class")  # specify measure here ("class" or "auc")
  if (nrow(.df) == 0) {return(NULL)}
  .df <- subset(.df, select = c(method, rule, groundtruth, n_eval, cv, measure, feats_type))
  return(.df)
})

.GenCompTableGE <- function(.lelem) {
...
table_list <- lapply(df_list, .GenCompTableGE)
...
sink("8-sim-results/results-out/lasso-pairwise-n200.txt")
mts[order(mts$lbtr), ]
mtm[order(mtm$lbtr), ]


.GenCompTableG <- function(.lelem) {
  .lelem$rank <- rank(1-.lelem$groundtruth, ties.method = "max")
  Y <- expand.grid(.lelem$rank, .lelem$rank)
  methods <- paste(.lelem$method, .lelem$rule, .lelem$n_eval, .lelem$cv, .lelem$measure, sep = "+")
  n_methods <- length(methods)
...
table_list <- lapply(df_list, .GenCompTableG)
table_df <- do.call(rbind, table_list)
table_df$comparison <- paste0(table_df$method1, " > ", table_df$method2)
...
sink("8-sim-results/results-out/lasso-pairwise-n400.txt")
mts[order(mts$lbtr), ]
mtm[order(mtm$lbtr), ]
