...
EstCis <- function(fpath, alpha, n_boots, n_cores) {
  ...
  foreach::foreach(fn = filenames, .packages = c("dplyr")) %dopar% {
    ...
    bounds <- rbind(std, proposed_tilt)
...
# system.time(EstCis(fpath = "class/normal/n200/cv/", alpha = alpha, n_boots = 10000, n_cores = n_cores))

system.time(EstCis(fpath = "class/normal/n400/cv/", alpha = alpha, n_boots = 10000, n_cores = n_cores))
